...
if [ -f ${file} ]; then
  ...
  for roinum in {1..90} ; do
    ...
    if [ -f ${subject}_FWNMT_hammers_native_roi${padroi}.txt ]; then
      ...
      fslmeants -i ${file} -m hammers/${roinum}.nii.gz >> ${subject}_FWNMT_hammers_native_roi${padroi}.txt

      paste *_FWNMT_hammers_native_roi*.txt > all_FWNMT_hammers.txt
    ...
    for roinum in {1..90} ; do
      ...
      if [ -f ${subject}_FWNMT_hammers_native_roi${padroi}.txt ]; then
        ...
        echo "NA" >> ${subject}_FWNMT_hammers_native_roi${padroi}.txt
        paste *_FWNMT_hammers_native_roi*.txt > all_FWNMT_hammers.txt
      done
    fi
